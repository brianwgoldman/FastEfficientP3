...
ggsave("rebuilds.eps", make_plot(data, "rebuilds*length/(cross+local)"), width=6, height=6)
ggsave("donations.eps", make_plot(data, "donation_attempts/(cross+local)"), width=6, height=6)
ggsave("cross.eps", make_plot(data, "cross/(cross+local)"), width=6, height=6)
ggsave("cross-success.eps", make_plot(data, "successes/(ties+successes+failures)") + log_y, width=6, height=6)
